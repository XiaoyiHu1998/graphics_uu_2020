...
void ObjectStorage_2D::init(){
    ...
    objects = std::vector<std::shared_ptr<Circle_2D>>{
            ...
            std::make_shared<Circle_2D>(sf::Vector2f(240,170), 6),
            std::make_shared<Circle_2D>(sf::Vector2f(20,247), 12),
            std::make_shared<Circle_2D>(sf::Vector2f(431,206), 24),
            std::make_shared<Circle_2D>(sf::Vector2f(253,302), 10)
    };
}